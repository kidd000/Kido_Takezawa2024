...
def run():
    ...
    # Global変数-------------------------------------------------------
    DES_NORM_PSYCH_INIT_SIGMA = 0.25
    ...
    def main(
        ...
        mean_des_normPsych_ls = np.zeros(MAX_GEN, dtype="float")
        ...
        # 【変更ポイント】同調の外適応の仮定を置かない　
        des_normPsych_arr = np.random.rand(GROUP_NUM, GROUP_SIZE) * 0.05
        # # Global変数-------------------------------------------------------
        ...
        for generation in range(MAX_GEN):
            ...
            for group_num in range(GROUP_NUM):
                ...
                # tgt_grp_str_X_arr = np.random.randint(0, high=2, size=GROUP_SIZE)
                tgt_grp_str_Y_arr = str_Y_arr[group_num, :]
                # tgt_grp_str_Y_arr = np.random.randint(0, high=2, size=GROUP_SIZE)
                tgt_grp_inj_normPsych_arr = inj_normPsych_arr[group_num, :]
                tgt_grp_des_normPsych_arr = des_normPsych_arr[group_num, :]
                ...
                for round_num in range(ROUND_NUM):
                    ...
                    if mask_isRevised.any():
                        mask_isRevised_idxes = [i for i, x in enumerate(mask_isRevised) if x]
                        for idx in mask_isRevised_idxes:
                            tgt_grp_str_X_arr[idx], tgt_grp_str_Y_arr[idx] = twoNormPsych_utility_max(str_X=tgt_grp_str_X_arr[idx], str_Y=tgt_grp_str_Y_arr[idx],
                                                                                                      inj_normPsych=tgt_grp_inj_normPsych_arr[idx], des_normPsych=tgt_grp_des_normPsych_arr[idx], num_X=num_X, num_Y=num_Y)

            # fitnessの計算 格納　0以上1になるように
            Fitness_arr = np.zeros((GROUP_NUM, GROUP_SIZE), dtype="float")
            for group_num in range(GROUP_NUM):
                Fitness_arr[group_num, :] = 1 + (cum_Payoff_arr[group_num, :] / ROUND_NUM)
            ...
            mean_des_normPsych_ls[generation] = np.mean(des_normPsych_arr)
            ...
            for grp_ID in range(GROUP_NUM):
                ...
                if offspring_group_count == 0:
                    ...
                    for _ in range(offspring_group_count):
                        ...
                        des_normPsych_arr[offspring_group_number, :] = new_grp_des_normPsych
            ...
            for grp_ID in range(GROUP_NUM):
                ...
                prev_ind_des_normPsych = np.copy(des_normPsych_arr[grp_ID, ])
                # 前の世代から垂直伝達 -------------------------------------------------------------------------
                ...
                for group_member_id in range(GROUP_SIZE):
                    ...
                    if offspring_count == 0:
                        ...
                        new_ind_des_normPsych = prev_ind_des_normPsych[group_member_id]
                        # 前の世代から垂直伝達 -------------------------------------------------------------------------
                        ...
                        for _ in range(offspring_count):
                            ...
                            des_normPsych_arr[grp_ID][offspring_number] = new_ind_des_normPsych
                            # 前の世代から垂直伝達 -------------------------------------------------------------------------
            ...
            if mask_des_isMutated.any():
                for mutated_mem in range(mask_des_isMutated.sum()):
                    idx_grp_arr = mask_des_isMutated.nonzero()[0]
                    idx_mem_arr = mask_des_isMutated.nonzero()[1]
                    des_normPsych_arr[idx_grp_arr[mutated_mem]][idx_mem_arr[mutated_mem]] = Mutation(des_normPsych_arr[idx_grp_arr[mutated_mem]][idx_mem_arr[mutated_mem]])
            # mutation ---------------------------------------
            ...
            migrate_des_arr = np.zeros((GROUP_NUM, GROUP_SIZE//2), dtype="float")
            # 前の世代から垂直伝達 -------------------------------------------------------------------------
            ...
            for group_num in range(GROUP_NUM):
                ...
                migrate_des_arr[group_num, :] = des_normPsych_arr[group_num][migrate_index_arr[group_num, :]].flatten()
                # 前の世代から垂直伝達 -------------------------------------------------------------------------
            ...
            np.random.shuffle(migrate_des_arr)
            # 前の世代から垂直伝達 -------------------------------------------------------------------------
            ...
            for group_num in range(GROUP_NUM):
                inj_normPsych_arr[group_num, :] = np.append(np.delete(inj_normPsych_arr[group_num], migrate_index_arr[group_num, :]), migrate_inj_arr[group_num * GROUP_SIZE // 2:(group_num + 1) * GROUP_SIZE // 2])
                des_normPsych_arr[group_num, :] = np.append(np.delete(des_normPsych_arr[group_num], migrate_index_arr[group_num, :]), migrate_des_arr[group_num * GROUP_SIZE // 2:(group_num + 1) * GROUP_SIZE // 2])
                # 前の世代から垂直伝達 -------------------------------------------------------------------------
    ...
    for Res_label, Res in Res_dir.items():
        np.savetxt(fname=f"./{head_chr}_{dir_name}/{Res_label}_{dir_name}_{RUN}.csv", X=Res, delimiter=",", fmt='%.5f')
